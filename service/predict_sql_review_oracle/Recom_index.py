...
# on 2019/8/2
# last modified on 2019/8/28 13:52

from service.predict_sql_review_oracle.Utility.Architecture import REC_FUNC, TransmitContainer, VerboseUnit
from service.predict_sql_review_oracle.Utility.Commu_buffer import ComBuffer
...
class RecomIndex(REC_FUNC):
                ...
                for id in children_ids:
                    ...
                    if child.type == 'COL':
                        ...
                        if tab_name.upper() not in out.keys():
                            out[tab_name.upper()] = [
                                [col_name.upper(), str(is_placeholder), opt.name, pu.find_add_hint_loc(opt, sem_info)]]
                        else:
                            out[tab_name.upper()].append(
                                [col_name.upper(), str(is_placeholder), opt.name, pu.find_add_hint_loc(opt, sem_info)])
        for k in out.keys():
        ...
        if tab_alias in self.table_alias.keys():
            ...
            for id in index_info.index:
                ...
                while len(index_cols) > 0:
                    ...
                    if not found:
                        break
                if index_info[index_info['INDEX_NAME'] == index_name]['UNIQUENESS'].values[0] != 'UNIQUE':
            ...
            if best_index is not None and level != -1:
                return best_index.upper(), max_value, max_id
            else:
                return ('', 0, -1)
        else:
            # -- handle view -- #
    ...
    def __index_builder(self, col_list_: np.array, tab_alias: str) -> Optional[Tuple[List[str], float, int]]:
        ...
        '''
        if tab_alias in self.table_alias.keys():
            tab = self.table_alias[tab_alias].upper()
            tabInfo = self.param['tab_info']
            row_num = tabInfo.get_tab_numrows(tab)
            col_level = dict()
            best_combo = None
    ...
    def __verbose(self, key: str, **kwargs) -> str:
        ...
        if key.upper() == 'BUILD_INDEX':
            ...
            recom += '\t建立索引的DDL语句为： CREATE INDEX xxx(索引名) ON ' + kwargs['tab_name'] + \
                     ' ( ' + index_list[0:-1] + ' )'
