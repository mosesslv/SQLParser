...
class Eval:
    ...
    def remove_virtual_index(self, tab_name: str):
        ...
        for col_combs, index_name in tuple_list:
            print(colored(f'\t{index_name:s}', 'yellow'))
            self._remove_virtual_index(schema_name=tab, index_name=index_name)
    ...
    def compared_explain(self, explain_old: pd.DataFrame, explain_new: pd.DataFrame) -> float:
        # compare the imporved sql(with index) with the origianl sql in terms of their #execution cost#
    ...
    def run(self) -> EvaluatorPackage:
        ...
        for i, id in enumerate(sql_id_checker):
            ...
            sql_text = sp.add_schema_name(tab.split('.')[1]) #添加schema name，如果未添加成功
                                                             # 会报错"[ERROR]- oracle execute error: ORA-00942: table or view does not exist"
            try:
