# -*- coding: UTF-8 -*-

"""
为 AI 提供附加信息数据, 可扩展
"""

from common.OracleHandle import OracleHandle, OracleHandleInfo
from service.AISQLReview.sql_abstract import OracleSQLStruct


class OracleAddition:
    """
    为 AI 提供 oracle 附加数据
    """
    def __init__(self, oracle_sql_struct):
        if oracle_sql_struct is None:
            raise Exception("Oracle sql struct invalid")

        assert isinstance(oracle_sql_struct, OracleSQLStruct)
        self._oracle_sql_struct = oracle_sql_struct
        self.last_error_info = ""

    def get_column_discrimination_data(self, data_dict):
        """
        获取单个字典数据的区分度信息
        :param data_dict: {"columns": ["a", "b"], "sql_text": "select * from dual"}
        :return: int (-1:无效值)
        """
        try:
            columns = data_dict["columns"]
            sql_text = data_dict["sql_text"]
        except Exception as ex:
            self.last_error_info = "[{0}] data invalid, {1}".format(data_dict, ex)
            return -1

        assert isinstance(self._oracle_sql_struct.oracle_conn, OracleHandle)
        disc_result = self._oracle_sql_struct.oracle_conn.ora_execute_query_get_all_data(sql_text)
        assert isinstance(disc_result, OracleHandleInfo)
        if not disc_result.result:
            self.last_error_info = "[{0}] data failed, {1}".format(data_dict, disc_result.message)
            return -1

        value = disc_result.data[0][0]

        # 附加信息也记录在 OracleSQLStruct 对象
        oracle_addition_dict = self._oracle_sql_struct.addition
        if oracle_addition_dict is None:
            oracle_addition_col_disc = []
            oracle_addition_dict["COLUMNS_DISCRIMINATION"] = oracle_addition_col_disc
        else:
            try:
                oracle_addition_col_disc = oracle_addition_dict["COLUMNS_DISCRIMINATION"]
            except KeyError:
                oracle_addition_col_disc = []
                oracle_addition_dict["COLUMNS_DISCRIMINATION"] = oracle_addition_col_disc
            except Exception as ex:
                self.last_error_info = "[{0}] handle key [COLUMNS_DISCRIMINATION] exception [{1}]".\
                    format(oracle_addition_dict, ex)
                return -1

        addition_data = {
            "columns": columns,
            "sql_text": str(sql_text),
            "value": disc_result.data
        }
        oracle_addition_col_disc.append(addition_data)
        return value
