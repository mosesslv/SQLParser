...
# --- parameters --- #
TAB_STRATEGY = 'ai_sr_tab_strategy'
SHAREPOOL = 'ai_sr_oracle_sharepool_src'


class Reporter:

    def __init__(self, builder_packge: BuilderPackage, eval_package: EvaluatorPackage, sqlId_freq: Dict[str, float]):
        self.error = dict()
    ...
    def report_sql_performance(self) -> Tuple[str, List[dict]]:
        ...
        for id in df.index:
            ...
            out_dict['sql_text'] = data['sql_text']
    ...
    def report_overall(self) -> Tuple[str, Optional[List[dict]]]:
        ...
        for insnt_schm_tab_name in self.builder_package:
            ...
            str_buffer += f'\t其中有 {pos:.2f}% 的sql性能提高, {med:.2f}% 的sql性能不变, {neg:.2f}% 的sql性能变差\n\n'
            e_dict[
    ...
    def run(self) -> str:
        ...
        sql_in_detail, sql_detial_list = self.report_sql_performance()
