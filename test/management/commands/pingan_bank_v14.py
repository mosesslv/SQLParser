...
DEFAULT_CONFIG = {
    ...
    'type': {
        'abbr': '-t',
        'default': 'AVGTIME',
        'help': 'type include [AVGTIME | CPU | IO]',
    },
    'count': {
        'abbr': '-c',
        'default': 1000,
        'help': 'count in [1~6000]',
    },
    'flushsharepool': {
        'abbr': '-f',
        'default': 'no',
    ...
    'snapshot': {
        'abbr': '-s',
        'default': '',
        'help': 'use with --flushsharepool=no'
    },
    'execaddr': {
        'abbr': '-e',
        'default': None,
    ...
    'cleantab': {
        'abbr': '-cl',
        'default': 'n',
        'help': 'clean tabStrategy [ y | n ]',
    },
    'output': {
        'default': 'all',
        'help': 'output type, [txt | html |all], default `all`'
    },
    'scan_tab_size': {
        'default': 100000,
    },
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser): #命令行调用
        for name, conf in DEFAULT_CONFIG.items():
            ...
            if 'abbr' in conf:
                ...
                # remove abbr from conf
                conf = conf.copy()
                conf.pop('abbr')
            
            args += ('--' + name,)
            
            # do not set default value for cli
            kw = conf.copy()
            kw.pop('default')
            parser.add_argument(*args, **kw) # 无显示调用，后台用隐式传入(django)
        

    def handle_snapshot_v14(self, username, password, instance_name, host_exc, host_actual, port, type, count,
                            snapshot, output, scan_tab_size, lift_threshold):
        verbose = {'AVGTIME': f'选择前{count}个平均运行时间最久的sql',
                   'CPU': f'选择前{count}个CPU占用最大的sql',
                   'IO': f'选择前{count}个IO最频繁的sql'}

        banksp =BankSharePoolHandle(username=username,password=password,
                                     instance_name=instance_name,
                                     ip_exec=host_exc,
                                     ip_actual=host_actual,
                                     port=port,
                                     clean=self.clean,
                                     scan_tab_size=scan_tab_size)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        # 生成报告O(3n)
                        Reporter(builder_pack, eval_pack, SQL_FREQ, lift_threshold = lift_threshold).run(output_type = output)
    ...
    def handle(self, *args, **options):
        # read config.ini params
        from configparser import ConfigParser
        config = ConfigParser()
        config.read('config.ini')
        
        conf = {k: config['PINGAN_BANK_V14'].get(k, v['default']) for k, v in DEFAULT_CONFIG.items()}

        # clean options from cli
        ...
        instance_name = conf.get('instance')
        address = conf.get('address')
        port = conf.get('port')
        # batch_number = options.get('number')
        # parallel = options.get('parallel')
        type = conf.get('type')
        count = int(conf.get('count'))
        flushsharepool = conf.get('flushsharepool')
        snapshot = conf.get('snapshot')
        execaddr = conf.get('execaddr')

        # --
        self.clean = True if conf.get('cleantab') == 'y' else False
        ...
        self.handle_snapshot_v14(username=username,
                                 ...
                                 count=count,
                                 snapshot=version,
                                 output=conf.get('output').upper(),
                                 scan_tab_size=int(conf.get('scan_tab_size')),
                                 lift_threshold = float(conf.get('lift_threshold')),
                                 )
...
class BankSharePoolHandle:
    def __init__(self, username, password, instance_name, ip_exec, ip_actual, port, clean, scan_tab_size):
        ...
        self.clean = clean
        self.scan_tab_size = scan_tab_size
...
where TABLE_NAME = '{2}'
    ...
    def call_tab_strategy(self, sql_result: Tuple):
        ...
        try:
            collector.run(handle_bank, scan_tab_size = self.scan_tab_size)
