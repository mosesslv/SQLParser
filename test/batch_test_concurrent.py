...
def Run_by_id(id: int) -> None:
    ...
    sql = f'select id,sql_text,plan_text,plan_raw,sql_data from ai_sr_sql_detail_sample' \
          f' where id = {id:d}'
...
def Run_test(num_process: int = 8) -> dict:
    server = SQL_SERVER_ADDRESS[SERVICE_ADD]
    sql = f'select id from ai_sr_sql_detail_sample where db_type={DB_TYPE:s}'
    ...
    for cnt,id in enumerate(id_list):
        #if DB_TYPE == '\'ORACLE\'':
        pool.apply_async(partial(Run_single_thread, MODEL=MODEL, RECOM=RECOM), (id,),
...
def save_to_dict(input: Tuple[dict, bool], result: dict, cnt: int, total_size: int, process_start: float):
    ...
    np.save(f'result_{DB_TYPE:s}.npy', result)
...
def Run_single_thread(id: int, MODEL: "AI", RECOM: "RECOMM") -> Tuple[dict, bool]:
    result = dict()
    server = SQL_SERVER_ADDRESS[SERVICE_ADD]
    sql = f'select id,sql_text,plan_text,plan_raw,sql_data,db_type from ai_sr_sql_detail_sample' \
          f' where id = {id:d}'
    pd_data = GetSQL(sql, server).get_data()
    success = True
...
def report():
    server = SQL_SERVER_ADDRESS[SERVICE_ADD]
    sql = 'select db_type, count(*) from ai_sr_sql_detail_sample group by db_type'
    pd_data = GetSQL(sql, server).get_data()
    print('=====================')
    print(pd_data)
    print('=====================')
...
if __name__ == '__main__':
    ...
    run_analysis = 0  # False
    report()
    if run_analysis:
        result = Run_test(1)
        analysis(result)
    else:
        result = np.load(f'result_{DB_TYPE:s}.npy', allow_pickle=True).item()
        analysis(result)
        value_list = result['FAIL']

        Run_by_id_list(value_list)
        #Run_by_id_list([30570])
        # Run_by_id_list([9288])
